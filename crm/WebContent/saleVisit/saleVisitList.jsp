...
<html>
	...
	<body>
		<form action="${pageContext.request.contextPath }/saleVisitAction_saleVisitList" method="post" id="listForm">
			...
			<input type="button" value="筛选" onclick="getSaleVisitList()" />
			<table>
				<thead>
					<tr>
						<td>业务员名称</td>
				...
				<tbody>
					<s:iterator value="#pageBean.list" var="saleVisit">
						<tr>
							<td><s:property value="#saleVisit.user.codeName"/></td>
							...
							<td><a href="${pageContext.request.contextPath }/saleVisitAction_getById?visit_id=<s:property value="#saleVisit.visit_id"/>">修改</a></td>
							<td><a href="${pageContext.request.contextPath }/saleVisitAction_deleteById?visit_id=<s:property value="#saleVisit.visit_id"/>">删除</a></td>
						</tr>
					</s:iterator>
				</tbody>
			</table>
			<div class="pageBar">
				<ul>
					<li onclick="getSaleVisitList(<s:property value="#pageBean.currentPage - 1"/>)">«</li>
					<s:iterator var="counter" begin="1" end="#pageBean.totalPage" step="1" >
						<li onclick="getSaleVisitList(<s:property value="counter"/>)" class=<s:property value="#counter == #pageBean.currentPage?'selected':''"/> >
				...
				每页显示：
				<select name="pageSize" oninput="getSaleVisitList()">
				...
				共<span>5</span>页&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="number" min="1" max="5" name="currentPage" value="${pageBean.currentPage }" onchange="getSaleVisitList()" id="formCur" />
	...
	<script>
		...
		//点击筛选按钮后访问数据
		function getSaleVisitList(currentPage){
			if(currentPage != null){
